...
import './App.css';
import { ReactComponent as Heart } from '../src/images/heartbroken.svg';
import Carregando from '../src/images/Spinner-1.6s-200px.gif';
...
function App() {
  ...
  function ListaPessoas(props) {
    ...
    const listagemPessoas = pessoas.map((pessoa) =>
      <img key={pessoa.id} src={pessoa.urls[4][512]} title="Pessoa gerada via IA" alt="pessoa gerada via IA" />
    );
    return (
      <>{listagemPessoas}</>
    );
